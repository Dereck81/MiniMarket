...
const Pedidos = () => {
    ...
    const handleDelete = async (idPedido) => {
        try {
            await axios.delete(`${ApiPath.apiBaseUrl}pedidos/${idPedido}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchPedidos();
        } catch (error) {
                    ...
                    <TableBody>
                        {pedidos.filter(pedido => pedido.pedido.usuario?.nombre?.toLowerCase().includes(search.toLowerCase())).map((pedido) => (
                            <TableRow key={pedido.pedido.idPedido}>
                                ...
                                <TableCell
                                    sx={{
                                        color: pedido.pedido.estado ?  'green' : 'red' ,
                                        textAlign: 'center',
                                        fontWeight: 'bold',
                                    }}
                                >
                                    {pedido.pedido.estado ? 'Activo' : 'Finalizado'}
                                </TableCell>
                                <TableCell>
                                    ...
                                    <IconButton
                                        onClick={() => handleDelete(pedido.pedido.idPedido)}
                                        color={pedido.pedido.estado ?  'success' : 'error'}
                                    >
                                        {pedido.pedido.estado ?  <ToggleOffIcon /> : <ToggleOnIcon /> }
                                    </IconButton>
                ...
                <Box sx={modalStyle}>
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField
                                {...params}
                                label="Seleccionar usuario con su DNI"
                                variant="outlined"
                                sx={{ marginBottom: 2 }}
                                required
                                fullWidth
                    ...
                    <Autocomplete
                        options={productos.filter((producto) =>
                            !selectedProductos.some(p => p.idInventario === producto.idInventario)
                        )}
                        ...
                        onChange={(event, newValue) => {
                            if (newValue && !selectedProductos.some(p => p.nombre === newValue.producto.nombre)) {
                                setSelectedProductos(prev => [
                                    ...prev,
                                    { nombre: newValue.producto.nombre, idInventario: newValue.idInventario, precioUnitario: newValue.precioVenta }
                                ]);
                    ...
                    >
                        {selectedProductos.map((producto) => (
                            <Box
                                key={producto.idInventario || producto.nombre}
                                sx={{
                                    display: 'flex',
                                    flexDirection: 'row',
                                    alignItems: 'center',
